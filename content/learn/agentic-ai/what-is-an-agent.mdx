...
"Agent" is the most abused word in enterprise AI right now. Every vendor demo calls its chatbot an agent, every automation platform calls its cron job an agent, and every team claiming "we're building agents" is usually building something simpler (or more complicated) than they think. Before this 90-lesson track can teach you to *build* agentic systems, it has to teach you what one *is*. This lesson is the foundation: three definitions that will organize everything after it, a decision rule for picking the right one, and a worked example from a real production pipeline.
...
Below workflows sits the oldest rung: **rules systems**. Google Cloud's definition of the agent spectrum puts it plainly: bots "follow pre-defined rules; limited learning; basic interactions," while agents "perform complex, multi-step actions; learn and adapt; can make decisions independently." [Source: https://cloud.google.com/discover/what-are-ai-agents] A rules system is `if (condition) then (action)`: deterministic, auditable, and zero percent flexible. A workflow inserts LLM calls *into* those predefined paths. An agent hands the path itself to the model.
...
Strip away the marketing and an agent is one tight loop. Google Cloud lists the core capabilities (**reasoning, acting, observing, planning, collaborating, self-refining** [Source: https://cloud.google.com/discover/what-are-ai-agents]), and TrueFoundry's 2026 engineering guide compresses them into the familiar sense-think-act cycle: the agent receives a goal, plans, selects and invokes tools, observes the result, evaluates whether it's done, and iterates until the objective is met or a termination condition fires. [Source: https://www.truefoundry.com/blog/llm-agents]

Two details matter for anyone about to build one:

1. **Tools are what make it an agent.** An LLM that only returns text answers your question; an agent calls search, databases, APIs, or code interpreters as part of the loop. The agent decides *which* tool and *when*; that's the "dynamic direction" in Anthropic's definition. The reasoning-and-acting pattern is formalized in the ReAct framework. [Source: https://arxiv.org/pdf/2210.03629]
2. **Autonomy is bounded.** Anthropic's agents "typically [are] just LLMs using tools based on environmental feedback in a loop," but the loop needs stop conditions, or it runs forever and spends your budget doing it. [Source: https://www.anthropic.com/engineering/building-effective-agents]
...
- **Workflow, orchestrator-workers variant:** the kanban pipeline. Kelex (the orchestrator) decomposes a client project into tasks and dispatches them to Steel/Lara/Zod: a central LLM dynamically breaking down work and synthesizing results, which is Anthropic's **orchestrator-workers** pattern. [Source: https://www.anthropic.com/engineering/building-effective-agents] Important nuance: even the orchestrator pattern is technically a *workflow*; the topology is predefined even though the subtasks aren't.
...
MIT Sloan adds the enterprise reality check: in a study of an agent that detects adverse events from clinical notes, **80% of the work was data engineering, stakeholder alignment, governance, and workflow integration, not prompt engineering or model tuning.** [Source: https://mitsloan.mit.edu/ideas-made-to-matter/agentic-ai-explained] The hard part of agentic AI is rarely the agent.
...
(Answers, per Anthropic's taxonomy: 1 is rules, no LLM. 2 is a routing workflow: the LLM classifies, but the path is predefined. 3 is an agent: the model directs its own process. 4 is the orchestrator-workers *workflow*, dynamic subtasks inside a fixed topology. [Source: https://www.anthropic.com/engineering/building-effective-agents])

## What's next

Lesson 2 is the building block under every agent: the **augmented LLM**: retrieval, tools, and memory, plus how Model Context Protocol standardizes the tool layer. Once you can reliably give a model tools, Lesson 3 walks the five workflow patterns in detail, and Lesson 4 finally assembles a real agent loop with stop conditions. You now have the vocabulary the whole track builds on, and you can already tell the difference between a demo and a real agent in the next vendor pitch.
...
- "What are AI agents? Definition, examples, and types" by Google Cloud (updated 04/02/2026): https://cloud.google.com/discover/what-are-ai-agents
- "Agentic AI, explained" by MIT Sloan (Kate Kellogg, Sinan Aral, John Horton): https://mitsloan.mit.edu/ideas-made-to-matter/agentic-ai-explained
- "What Are LLM Agents? A Complete Practical Guide" by TrueFoundry (June 14, 2026): https://www.truefoundry.com/blog/llm-agents
- "ReAct: Synergizing Reasoning and Acting in Language Models" by Yao et al. (arXiv): https://arxiv.org/pdf/2210.03629

*Note: framework names, model names, and vendor capabilities move fast; the Fortress incident described here is documented in internal ops notes (t_db6b0a22) as of August 2026; verify current tool-calling behavior against the model you actually deploy.*
