...
- **Pin actions to full commit SHAs** (`actions/checkout@<40-char-sha>`) instead of tags, and let Dependabot manage the updates so pinning doesn't stall your upgrades [^8] [^3]. Remember the honest caveat the community keeps raising: SHA pinning gives immutability, not provenance, so verify the SHA actually exists in the named repository, not an attacker-controlled fork [^4].
