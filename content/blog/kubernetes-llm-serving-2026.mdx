...
**Abstract above it.** Ray Serve gives a model-level abstraction on top of Kubernetes, trading one orchestration system for a runtime designed around stateful AI workloads. Managed endpoints like Replicate, Baseten, or Fireworks take GPUs off your plate entirely [^4].

**Be honest about scale.** If you serve under 10 concurrent users on a model under 13B parameters, Ollama on a decent GPU is fine, and a managed endpoint costs less than the engineering time to run vLLM [^8]. Self-hosting pays off at latency-sensitive or data-sovereign workloads at real volume, which we covered when we looked at when local inference earns its keep.
